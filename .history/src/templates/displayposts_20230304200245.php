...
<body>
        ...
        <!-- Post Slider -->
            ...
            <h1 class="posts-title">Articles</h1>
               ...
               <?php foreach($articles as $post):?>
                    ...
                    <img src ="<?php echo './public/image/'.$post['image']?> " alt="mon image" class="slider-image">
                    <div class="card-body">
                        <h4><a href="index.php?action=displayonepost&id=<?= $post['id']?>"><?= $post['title']?></a></h4>     
                        &nbsp;
                        <i><?php echo date( 'd F,Y',strtotime($post['created_at']));?></i><p>ECRIT dans la categorie:
                        <?php echo $post['name']?> par <?php echo $post['prenom'] ?></p>
                        <?php endforeach; ?>
  ...
  <div class="card-body">
    <h5 class="card-title"><a href="index.php?action=displayonepost&id=<?= $post['id']?>"><?= $post['title']?></a</h5>
    &nbsp;
                        <i><?php echo date( 'd F,Y',strtotime($post['created_at']));?></i><p>ECRIT dans la categorie:
                        <?php echo $post['name']?> par <?php echo $post['prenom'] ?></p> 
