...
function afficheComment($id_post,$user_id,$comment,$created_at){
    $db = connectionDataBase();
    $req ="SELECT comments.user_id, comments.id , comments.created_at,comments.comments,users.prenom,categories.name FROM comments
    LEFT JOIN users ON comments.user_id = users.id
    LEFT JOIN categories ON comments.id_post = categories.id";
     $stmt = $db->prepare($req);
}