...
function afficheComment($id_post){
    $db = connectionDataBase();
    $req ="SELECT users.prenom ,comments.user_id, comments.id , comments.created_at,comments.comment,comments.id_post FROM comments c
    LEFT JOIN users u ON comments.user_id = users.id
    LEFT JOIN posts p ON comments.id_post = posts.id  WHERE comments.id= $id_post";
    $stmt = $db->prepare($req);
    $stmt->execute();
