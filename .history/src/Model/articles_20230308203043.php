...
function afficheComment($id_post){
    $db = connectionDataBase();
    $req ="SELECT comments.user_id, comments.id , comments.created_at,comments.comments,comments.id_post FROM comments
    LEFT JOIN users ON comments.user_id = users.id
    LEFT JOIN posts ON comments.id_post = posts.id WHERE id_post = :post_id ORDER BY created_at DESC)";
    ;
    $stmt = $db->prepare($req);
    $stmt->execute(array('post_id' => $id_post));
    $comments = $stmt->fetch();
